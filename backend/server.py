"""FILINEX backend - leads, newsletter, blog, admin."""
...
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "Filinex@2026")

app = FastAPI(title="FILINEX API", version="1.0")
...
class LeadCreate(BaseModel):
    ...
    source: Optional[str] = "contact"  # contact | discovery | estimator | start_project
...
class Lead(BaseModel):
    ...
    message: Optional[str] = None
    source: str = "contact"
    status: str = "new"  # new | qualified | meeting_scheduled | proposal_sent | won | lost
...
@api_router.get("/")
async def root():
    return {"name": "FILINEX API", "status": "operational", "version": "1.0"}
...
async def leads_stats(_: str = Depends(require_admin)):
    ...
    counts = {}
    for s in statuses:
        counts[s] = await db.leads.count_documents({"status": s})
...
DEFAULT_POSTS = [
    ...
    },
    {
        ...
        "featured": True,
        "published": True,
        "content": "# Cinematic Launches\n\nA launch page is not a brochure — it is a film. Pacing, restraint, and contrast carry far more weight than copy. In this article we break down the motion grammar we use at FILINEX for every premium launch.\n",
    },
    {
        ...
        "featured": False,
        "published": True,
        "content": "# Web3 Without the Hype\n\nWe ship Web3 systems only when they solve a real coordination problem. Here is the decision tree we use with every client.\n",
    },
    {
